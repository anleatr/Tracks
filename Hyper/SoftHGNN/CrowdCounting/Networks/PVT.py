...
import math
from .SoftHGNN_module import SoftHGNN
from .SoftHGNN_SeS_module import SoftHGNN_SeS

class Regression(nn.Module):
    def __init__(self, add_module_choice='None'):
        super().__init__()
        ...
        self.v2 = nn.Sequential(
            ...
            nn.Conv2d(320, 256, 3, padding=1, dilation=1),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x1, x2, x3):
        ...
        x = self.fuse(x)
        if self.hyper_module is not None:
            B, C, H, W = x.shape
            x_flat = x.flatten(2).transpose(1, 2)
            out = self.hyper_module(x_flat)
...
class PyramidVisionTransformerV2(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], num_stages=4, linear=False, add_module_choice='None'):
        super().__init__()
        ...
        self.regression = Regression(add_module_choice)
        self.apply(self._init_weights)
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            trunc_normal_(m.weight, std=.02)
    ...
    def forward(self, x):
        ...
        mu, lb_loss = self.regression(x[1], x[2], x[3])
        B, C, H, W = mu.size()
        mu_sum = mu.view([B, -1]).sum(1).unsqueeze(1).unsqueeze(2).unsqueeze(3)
        mu_normed = mu / (mu_sum + 1e-6)
        if self.training:
            return mu, mu_normed, lb_loss
        else:
            return mu, mu_normed
...
@register_model
def pvt_v2_b3(pretrained=False, add_module='None', **kwargs):
    model = PyramidVisionTransformerV2(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 18, 3], sr_ratios=[8, 4, 2, 1], add_module_choice=add_module,
        **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        '''download from https://github.com/whai362/PVT/releases/download/v2/pvt_v2_b3.pth'''
