...
from Networks import ALTGVT, PVT
...
class Trainer(object):
    ...
    def setup(self):
        args = self.args
        sub_dir = (
            "CCtrans/{}_2025-input-{}_wot-{}_wtv-{}_reg-{}_nIter-{}_normCood-{}".format(
                args.run_name,
                args.crop_size,
                args.wot,
                args.wtv,
        ...
        if args.backbone == 'PVT':
            self.model = PVT.pvt_v2_b3(pretrained=True, add_module=args.add_module)
        elif args.backbone == 'ALTGVT':
            self.model = ALTGVT.alt_gvt_large(pretrained=True, add_module=args.add_module)
        else:
        ...
        if args.wandb:
            ...
            config=args, project="CCTrans", name=args.run_name
        )
        else : 
    ...
    def train_epoch(self):
        ...
        for step, (inputs, points, gt_discrete) in enumerate(self.dataloaders["train"]):
            ...
            with torch.set_grad_enabled(True):
                outputs, outputs_normed, lb_loss = self.model(inputs)
                # Compute OT loss.
                ...
                loss = ot_loss + count_loss + tv_loss + lb_loss
